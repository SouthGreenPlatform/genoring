...
################################################################################
# GenoRing main script.
# v1.0
#
# This script manages GenoRing platform. See help for details using the command
# line:
#
#  ./genoring.sh help
#
...
BASEDIR=$(dirname $0)
VERSION=1.0
EXTENSION_FILE=extensions.conf
...
genoring_enable () {
  ...
  if [ -z "$extension_status" ]; then
    ...
    exit 1
  elif [ "0" == "$extension_status" ]; then
    # Enable specified extensions.
    # Add extension to the enabled extension file.
    echo "$1" >> ./enabled_extensions.txt
    # Copy nginx config.
    if [ -e "./extensions/$1/nginx/$1.conf" ]; then
      # Do not overwrite existing.
      cp -n "./extensions/$1/nginx/$1.conf" ./proxy/extensions/
    fi
    # Call init scripts.
    echo "Initializing..."
    if [ -x "./extensions/$1/hooks/init.sh" ]; then
      echo "- $1"
      ./extensions/$1/hooks/init.sh
    fi
    if [ -d "./extensions/$1/hooks" ]; then
      ...
      for scriptname in extensions/$1/hooks/init_*.sh; do
        ...
        if [ ! -z "$container_is_running" ]; then
          echo "- $container_name"
          docker exec -v ./extensions/$1/hooks/init_$container_name.sh:/usr/init/init_$container_name.sh -it $container_name /user/init/init_$container_name.sh
        fi
      done
    fi
    echo "...initialization done."
  elif [ "1" == "$extension_status" ]; then
...
genoring_disable () {
  ...
  if [ -z "$extension_status" ]; then
    ...
    # Remove nginx config.
    if [ -e "./proxy/extensions/$1.conf" ]; then
      rm ./proxy/extensions/$1.conf 
    fi
    # Call uninstall scripts.
    echo "Uninstalling..."
    if [ -x "./extensions/$1/hooks/uninstall.sh" ]; then
      echo "- $1"
      ./extensions/$1/hooks/uninstall.sh
    fi
    if [ -d "./extensions/$1/hooks" ]; then
      ...
      for scriptname in ./extensions/$1/hooks/uninstall_*.sh; do
        ...
        if [ ! -z "$container_is_running" ]; then
          echo "- $container_name"
          docker exec -v ./extensions/$1/hooks/uninstall_$container_name.sh:/usr/init/uninstall_$container_name.sh -it $container_name /user/init/uninstall_$container_name.sh
        fi
      done
    fi
...
get_genoring_extensions () {
  if [ -z $1 ]; then
    find extensions -mindepth 1 -maxdepth 1 -type d -printf "%f\n"
  elif [ "0" == "$1" ]; then
    find extensions -mindepth 1 -maxdepth 1 -type d -printf "%f\n" | grep -v -x -f ./enabled_extensions.txt
  elif [ "1" == "$1" ]; then
...
get_extension_status () {
  ...
  extension_status=
  if [ ! -z "$(grep -x $1 ./enabled_extensions.txt)" ]; then
    extension_status=1
  elif [ ! -z $(find extensions -mindepth 1 -maxdepth 1 -type d -printf "%f\n" | grep -v -x $1) ]; then
    extension_status=0
  fi
}
...
##
# Makes sure current user is admin.
#
assert_root () {
  if [ "$EUID" -ne 0 ]; then
    >&2 echo "STOP: This command needs to be run as root."
    exit 1
  fi
}
...
case "$1" in
  ...
  autotests)
    # container_is_running()
    echo "Testing container_is_running()..."
    echo -n "  - test 1: "
    container_is_running 2>/dev/null
    if [ ! -z "$container_is_running" ]; then
      >&2 echo -e "FAILED\nERROR: container_is_running failed test 1: no parameters\nValue: $container_is_running"
      exit 1
    fi
    echo "OK"
    echo -n "  - test 2: "
    container_is_running genoring-test-ZZ
    if [ ! -z "$container_is_running" ]; then
      >&2 echo -e "FAILED\nERROR: container_is_running failed test 2: non-existing container\nValue: $container_is_running"
      exit 1
    fi
    echo "OK"
    echo -n "  - test 3: "
    # Moch.
    docker () {
      echo -e 'CONTAINER ID   IMAGE                              COMMAND                  CREATED        STATUS        PORTS                                                NAMES\n0123456789ab   someone/genoring-test-ZZ:1.0   "command.sh"        18 hours ago   Up 18 hours   0.0.0.0:80->8080/tcp, :::80->8080/tcp            genoring-test-ZZ\n'
    }
    container_is_running genoring-test-ZZ
    if [ -z "$container_is_running" ]; then
      >&2 echo -e "FAILED\nERROR: container_is_running failed test 3: running container\nValue: $container_is_running"
      exit 1
    fi
    echo "OK"
    echo "...container_is_running() all tests passed."

    # get_env_setting()
    echo "Testing get_env_setting()..."
    echo -e "FIRST_SETTING=abc\nSECOND_SETTING : def ghi\nTHIRD_SETTING=xyz\nFORTH_SETTING='jkl mno pqr'\nTHIRD_SETTING=\nFIFTH_SETTING:\"'stuv'\"" > extensions/genoring/env/autotests.env
    echo -n "  - test 1: "
    get_env_setting extensions/genoring/env/autotests.env 'FIRST_SETTING'
    if [ -z "$env_value" ] || [ "abc" != "$env_value" ]; then
      >&2 echo -e "FAILED\nERROR: get_env_setting failed test 1: FIRST_SETTING\nValue: $env_value"
      rm extensions/genoring/env/autotests.env
      exit 1
    fi
    echo "OK"
    echo -n "  - test 2: "
    get_env_setting extensions/genoring/env/autotests.env 'SECOND_SETTING'
    if [ -z "$env_value" ] || [ "def ghi" != "$env_value" ]; then
      >&2 echo -e "FAILED\nERROR: get_env_setting failed test 2: SECOND_SETTING\nValue: $env_value"
      rm extensions/genoring/env/autotests.env
      exit 1
    fi
    echo "OK"
    echo -n "  - test 3: "
    get_env_setting extensions/genoring/env/autotests.env 'THIRD_SETTING'
    if [ ! -z "$env_value" ]; then
      >&2 echo -e "FAILED\nERROR: get_env_setting failed test 3: THIRD_SETTING\nValue: $env_value"
      rm extensions/genoring/env/autotests.env
      exit 1
    fi
    echo "OK"
    echo -n "  - test 4: "
    get_env_setting extensions/genoring/env/autotests.env 'FORTH_SETTING'
    if [ -z "$env_value" ] || [ "jkl mno pqr" != "$env_value" ]; then
      >&2 echo -e "FAILED\nERROR: get_env_setting failed test 4: FORTH_SETTING\nValue: $env_value"
      rm extensions/genoring/env/autotests.env
      exit 1
    fi
    echo "OK"
    echo -n "  - test 5: "
    get_env_setting extensions/genoring/env/autotests.env 'FIFTH_SETTING'
    if [ -z "$env_value" ] || [ "'stuv'" != "$env_value" ]; then
      >&2 echo -e "FAILED\nERROR: get_env_setting failed test 5: FIFTH_SETTING\nValue: $env_value"
      rm extensions/genoring/env/autotests.env
      exit 1
    fi
    echo "OK"
    echo -n "  - test 6: "
    get_env_setting extensions/genoring/env/autotests.env 'SETTING'
    if [ ! -z "$env_value" ]; then
      >&2 echo -e "FAILED\nERROR: get_env_setting failed test 6: FORTH_SETTING\nValue: $env_value"
      rm extensions/genoring/env/autotests.env
      exit 1
    fi
    echo "OK"
    echo "...get_env_setting() all tests passed."

    # set_env_setting()
    echo "Testing set_env_setting()..."
    echo -n "- test 1: "
    set_env_setting extensions/genoring/env/autotests.env THIRD_SETTING 1234
    get_env_setting extensions/genoring/env/autotests.env THIRD_SETTING
    if [ -z "$env_value" ] || [ "$env_value" != "1234" ]; then
      >&2 echo -e "FAILED\nERROR: set_env_setting failed test 1: THIRD_SETTING\nValue: $env_value"
      rm extensions/genoring/env/autotests.env
      exit 1
    fi
    echo "OK"
    rm extensions/genoring/env/autotests.env
    echo "...set_env_setting() all tests passed."
