...
**Q. Is it possible to modify `docker-compose.yml`? Is it possible to override
    `docker-compose.yml`?**

**A.** It is not recommended to edit `docker-compose.yml` as any update to
    GenoRing could regenerate a `docker-compose.yml` file without any manual
    modifications. Instead, you can create a `docker-compose.override.yml` file
    that would only contain the requested overrides that will be loaded even if
    the original `docker-compose.yml` file has been regenerated by GenoRing.
